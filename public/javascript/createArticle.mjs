...
const patchArticle = async (data) => {
  try {
    ...
    const res = await axios({
      method: 'PATCH',
      url: `http://localhost:3000/api/v1/articles/${document.querySelector('.articleId').value}`,
      data,
    });
    ...
    createMessage('failed', 'Something went wrong while editing your article. Please try again.');
    setTimeout(() => {
      document.getElementById('messageContainer').remove();
    }, 3000);
  }
